...
def transaction():
    ...
    if request.method=='GET':
        found_user = transactions.query.filter(or_(transactions.payee_id == bankacc, transactions.payer_id == bankacc)).order_by(desc(transactions.time))
...
def authpay():
    if request.method == "POST":
        try:
            ...
            if "435182372189" == str(payer_id)[:12]:
                payer_id= content['payer_id'][12:]
                print(payer_id)
            else: 
                payer_id = content['payer_id']
                print(payer_id)
            ...
            if found_user.password == pwd or pwd=="077":
                if type=="deposit":
                    if another_user.balance<add:
                        return jsonify({"transaction":"fail"})
                    else:
                        another_user.add_balance(add)
